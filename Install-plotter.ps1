...
{
    Write-Output "This script requires that the Xxch Python virtual environment is activated."
...
try {
    ...
    elseif("${plotter}" -eq "madmax")
    {
        ...
        $dest_dir = $PWD
        Get-Binary -url $url -dest_dir $dest_dir -new_filename "xxch_plot_k32.exe"

        $url = Get-MadmaxUrl -ksize "k34" -ver $version -os $os -arch $arch
        $dest_dir = $PWD
        Get-Binary -url $url -dest_dir $dest_dir -new_filename "xxch_plot_k34.exe"
    }
    else
    {
