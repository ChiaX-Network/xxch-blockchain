...
git submodule update --init xxch-blockchain-gui

Set-Location -Path ".\xxch-blockchain-gui" -PassThru
...
# Other than `xxch-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @xxch-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@xxch-network` because it causes an error on later `electron-packager` command
#Remove-Item "@xxch-network" -Recurse -Force
