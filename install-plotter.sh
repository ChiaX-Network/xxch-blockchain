...
get_madmax_filename() {
  ...
  export XXCH_PLOT="xxch_plot"
  if [ "$KSIZE" = "k34" ]; then
    export XXCH_PLOT="xxch_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${XXCH_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
