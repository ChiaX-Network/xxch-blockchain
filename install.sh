...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with xxch
  ...
  # yum install python3 brings Python3.6 which is not supported by xxch
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "12" ]; then
  echo "Xxch requires Python version >= 3.7 and  < 3.12.0" >&2
...
echo ""
echo "Xxch blockchain install.sh complete."
...
echo ""
echo "Type '. ./activate' and then 'xxch init' to begin."
