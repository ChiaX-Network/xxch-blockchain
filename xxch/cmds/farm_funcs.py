...
from xxch.util.misc import format_bytes, format_minutes
from xxch.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total xxch farmed: {amounts['farmed_amount'] / units['xxch']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['xxch']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['xxch']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'xxch start wallet' and 'xxch wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'xxch wallet show'")
    else:
        print("Note: log into your key using 'xxch wallet show' to see rewards for each key")
