...
from xxch.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path) -> None:
    from xxch.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'xxch plots add -d [dir]' and remove with"
        + " 'xxch plots remove -d [dir]'"
        + " Scan and check plots with 'xxch plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    from xxch.util.xxch_logging import initialize_logging

    root_path: Path = ctx.obj["root_path"]
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'xxch init'")
...
def create_cmd(
    ...
    from xxch.plotting.create_plots import create_plots, resolve_plot_keys
    from xxch.plotting.util import Params
...
def check_cmd(
    ...
    from xxch.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str) -> None:
    from xxch.plotting.util import remove_plot_directory
