...
from xxch.cmds.cmds_util import get_any_service_client
from xxch.cmds.start_funcs import async_start
from xxch.consensus.coinbase import create_puzzlehash_for_pk
from xxch.server.outbound_message import NodeType
from xxch.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from xxch.util.config import load_config, save_config, set_peer_info
from xxch.util.errors import KeychainFingerprintExists
...
def create_xxch_directory(
    ...
    """
    This function creates a new xxch directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from xxch.cmds.init_funcs import xxch_init

    if not xxch_root.is_dir() or not Path(xxch_root / "config" / "config.yaml").exists():
        # create xxch directories & load config
        xxch_init(xxch_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(xxch_root, "config.yaml")
        ...
        config = load_config(xxch_root, "config.yaml")
    # simulator overrides
    ...
    save_config(xxch_root, "config.yaml", config)
    return config


def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `xxch keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from xxch.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from xxch.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["XXCH_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create xxch directory & get config.
    print("Creating xxch directory & config...")
    config = create_xxch_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "xxch")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your XXCH_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from xxch.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically xxch show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['xxch']} TXXCH."
        )
        if show_addresses:
            print("All Addresses: ")
