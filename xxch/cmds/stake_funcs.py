...
from xxch.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg, get_wallet_client
...
from xxch.types.stake_value import STAKE_FARM_LIST, STAKE_LOCK_LIST
from xxch.util.bech32m import encode_puzzle_hash
from xxch.util.config import selected_network_address_prefix
from xxch.util.errors import CliRpcConnectionError
...
from xxch.wallet.util.query_filter import TransactionTypeFilter, HashFilter
...
def print_transaction(
    ...
    if verbose:
        ...
        xxch_amount = Decimal(int(tx.amount)) / units["xxch"]
        ...
        print(f"Amount {description}: {xxch_amount} {address_prefix.upper()}")
...
async def stake_info(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        if is_stake_farm:
            print(f"Stake Balance: {response['balance'] / units['xxch']}")
            print(f"Stake Balance Other: {response['balance_other'] / units['xxch']}")
            print(f"Stake Balance Income: {response['balance_income'] / units['xxch']}")
            print(f"Stake Balance 24H Exp: {response['balance_exp'] / units['xxch']}")
            print(f"Stake Reward: {response['stake_reward'] / units['xxch']}")
...
async def stake_send(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        final_amount = int(amount * units["xxch"])
        if not (final_amount / units["xxch"]).is_integer():
            print(
...
#             return
#         response = await wallet_client.spend_stake_coins(tx_ids, int(fee * units["xxch"]))
#         print(str(response))
...
async def find_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        total_amount = response["total_amount"] / units["xxch"]
        record_amount = response["record_amount"] / units["xxch"]
        balance_amount = response["balance_amount"] / units["xxch"]
        print(f"Contract Address: {address}")
        print(f"Total Amount: {total_amount} XXCH")
        print(f"Balance Amount: {balance_amount} XXCH")
        print(f"Record Amount: {record_amount} XXCH")
