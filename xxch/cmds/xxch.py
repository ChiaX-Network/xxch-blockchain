...
from xxch.cmds.farm import farm_cmd
from xxch.cmds.init import init_cmd
from xxch.cmds.keys import keys_cmd
from xxch.cmds.netspace import netspace_cmd
from xxch.cmds.passphrase import passphrase_cmd
from xxch.cmds.peer import peer_cmd
from xxch.cmds.plotnft import plotnft_cmd
from xxch.cmds.plots import plots_cmd
from xxch.cmds.plotters import plotters_cmd
from xxch.cmds.rpc import rpc_cmd
from xxch.cmds.show import show_cmd
from xxch.cmds.start import start_cmd
from xxch.cmds.stop import stop_cmd
from xxch.cmds.wallet import wallet_cmd
from xxch.cmds.stake import stake_cmd
from xxch.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from xxch.util.errors import KeychainCurrentPassphraseIsInvalid
from xxch.util.keychain import Keychain, set_keys_root_path
from xxch.util.ssl_check import check_ssl

CONTEXT_SETTINGS = dict(help_option_names=["-h", "--help"])


@click.group(
    help=f"\n  Manage xxch blockchain infrastructure ({__version__})\n",
    epilog="Try 'xxch start node', 'xxch netspace -d 192', or 'xxch show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        ...
        from xxch.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", help="Show xxch version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs xxch daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by xxch start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio

    from xxch.daemon.server import async_run_daemon
