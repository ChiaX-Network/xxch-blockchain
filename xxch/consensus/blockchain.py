...
from xxch.consensus.block_body_validation import validate_block_body
from xxch.consensus.block_header_validation import validate_unfinished_header_block
...
from xxch.consensus.find_fork_point import find_fork_point_in_chain
from xxch.consensus.full_block_to_block_record import block_to_block_record
from xxch.consensus.multiprocess_validation import (
    PreValidationResult,
    _run_generator,
    pre_validate_blocks_multiprocessing,
)
from xxch.consensus.pos_quality import UI_ACTUAL_SPACE_CONSTANT_FACTOR
from xxch.full_node.block_height_map import BlockHeightMap
from xxch.full_node.block_store import BlockStore
from xxch.full_node.coin_store import CoinStore
from xxch.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_spends_for_block
...
from xxch.types.blockchain_format.vdf import VDFInfo
...
from xxch.types.stake_value import get_stake_value, STAKE_PER_COEFFICIENT, STAKE_FARM_MIN, STAKE_LOCK_MIN
...
from xxch.types.weight_proof import SubEpochChallengeSegment
from xxch.util.errors import ConsensusError, Err
from xxch.util.generator_tools import get_block_header, tx_removals_and_additions
from xxch.util.hash import std_hash
from xxch.util.inline_executor import InlineExecutor
from xxch.util.ints import uint16, uint32, uint64, uint128
from xxch.util.priority_mutex import PriorityMutex
from xxch.util.setproctitle import getproctitle, setproctitle
from xxch.wallet.puzzles.stake.drivers import match_stake_puzzle_by_coin_spend
...
class Blockchain(BlockchainInterface):
    ...
    async def _reconsider_peak(
        ...
        if block_record.prev_hash != peak.header_hash:
            ...
            await self.stake_record_store.rollback_to_block(fork_height)
        ...
        for fetched_full_block, fetched_block_record in reversed(blocks_to_add):
            ...
            if coin_spends_res is not None:
                ...
                for coin in tx_additions:
                    if (coin.amount / MOJO_PER_XXCH).is_integer():
    ...
    async def get_tx_removals_and_additions(
        ...
        coin_spends = get_spends_for_block(block_generator) if block_generator is not None else None
        tx_removals, tx_additions = tx_removals_and_additions(npc_result.conds)
    ...
    async def get_stake_height_network_space(
        ...
        if height is not None and height > 1:
            ...
            return await self.get_network_space(newer_block_bytes, older_block_bytes)
        else:
            return uint128(0)
    ...
    async def get_stake_coefficient(self, height: uint32, farmer_public_key: G1Element) -> uint64:
        ...
        if height > 1 and header_hash is not None:
        ...
        if curr is not None and curr.timestamp is not None:
            block_range = 4608 * 14
            blocks = await self.block_store.get_reward_block_count(
                uint32(max(0, height - block_range)),
                height,
                stake_puzzle_hash,
            )
            ...
            # log.debug(
            #     f"height: {height}, stake: {stake_amount / MOJO_PER_XXCH}, coefficient: {coefficient}, "
            ...
            #         await session.post("http://192.168.18.19:21819/coefficient/xxch", data=json.dumps({
            #             "height": height,
            #             "staking": stake_amount_real,
            #             "staking_realy": stake_amount / MOJO_PER_XXCH,
            #             "difficulty": coefficient,
            #             "space": space / (1024 ** 4),
            #             "total_space": network_space / (1024 ** 4),
            #             "public_key": bytes(farmer_public_key).hex(),
            #             "address": encode_puzzle_hash(stake_puzzle_hash, STAKE_FARM_PREFIX),
            #             "blocks": blocks,
            #             "data": [record.to_json_dict() for record in stake_records],
            #             "timestamp": int(time.time()),
            #         }))
            # except Exception as e:
            #     log.debug(f"post_staking exception {e}")
        stake_coefficient = uint64(int(coefficient * STAKE_PER_COEFFICIENT))
    ...
    async def get_stake_lock_records(
        ...
        for stake in stake_records:
            ...
            if start == expiration or end == expiration:
                continue
            stake_rewards[stake.stake_puzzle_hash] = stake_rewards.get(
                stake.stake_puzzle_hash, 0
            ) + value.reward_amount(stake.amount)
        return stake_rewards
