...
from xxch.consensus.block_body_validation import ForkInfo, validate_block_body
...
from xxch.consensus.find_fork_point import lookup_fork_chain
...
class Blockchain(BlockchainInterface):
    ...
    async def _reconsider_peak(
        ...
        for fetched_full_block, fetched_block_record in reversed(blocks_to_add):
            ...
            height = fetched_full_block.height
            ...
            if coin_spends_res is not None:
                ...
                newStake = height >= STAKE_FORK_HEIGHT
                ...
                for coin_spend in coin_spends_res:
                    ...
                    if stake_metadata is not None:
                        if newStake and stake_metadata.stake_type < 20:
                            continue
                        ...
                        if coin is not None:
                            if coin.amount < STAKE_FARM_MIN if stake_metadata.is_stake_farm else (
                                ...
                                continue
                            stake_value = get_stake_value_old(
        ...
        if curr is not None and curr.timestamp is not None:
            ...
            if network_space != 0 and stake_amount > 0:
                if space == 0 or blocks == 0:
                    ...
                    coefficient = round(0.05 + 1 / (stake_amount / space / proportion + 0.05), 15)
        ...
        if len(stake_records) > 0:
            if height < STAKE_FORK_HEIGHT:
                for stake in stake_records:
                    ...
                    ) + int(value.least_reward_amount(stake.amount))
            else:
                stake_amount_total = await self.stake_record_store.get_stake_lock_amount_total(end) * MOJO_PER_XXCH
