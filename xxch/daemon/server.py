...
from xxch.cmds.init_funcs import check_keys, xxch_full_version_str, xxch_init
from xxch.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from xxch.consensus.coinbase import create_puzzlehash_for_pk
from xxch.daemon.keychain_server import KeychainServer, keychain_commands
from xxch.daemon.windows_signal import kill
from xxch.plotters.plotters import get_available_plotters
from xxch.plotting.util import add_plot_directory
from xxch.server.server import ssl_context_for_server
from xxch.util.bech32m import encode_puzzle_hash
from xxch.util.beta_metrics import BetaMetricsLogger
from xxch.util.xxch_logging import initialize_service_logging
from xxch.util.config import load_config
from xxch.util.errors import KeychainCurrentPassphraseIsInvalid
from xxch.util.ints import uint32
from xxch.util.json_util import dict_to_json_str
from xxch.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
from xxch.util.lock import Lockfile, LockfileError
from xxch.util.log_exceptions import log_exceptions
from xxch.util.misc import SignalHandlers
from xxch.util.network import WebServer
from xxch.util.service_groups import validate_service
from xxch.util.setproctitle import setproctitle
from xxch.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Xxch.")
    quit()


log = logging.getLogger(__name__)

service_plotter = "xxch_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "xxch_full_node_simulator": "xxch_simulator",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Xxch will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["xxch", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'xxch plotters bladebit ramplot' or 'xxch plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered XXCH_ROOT
    os.environ["XXCH_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with XXCH_ROOT: {os.environ['XXCH_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in xxch_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    xxch_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("xxch_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"xxch-blockchain version: {xxch_full_version_str()}")
