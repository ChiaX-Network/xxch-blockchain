...
from xxch.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from xxch.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from xxch.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from xxch.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
...
from xxch.wallet.util.merkle_utils import _simplify_merkle_proof
from xxch.wallet.util.transaction_type import TransactionType
from xxch.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from xxch.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSXxchConnection) -> CoinState:
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, xxch_tx.spend_bundle])
                ),
                dataclasses.replace(xxch_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSXxchConnection, coin_data: Optional[object]) -> None:
