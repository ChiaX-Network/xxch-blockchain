...
from xxch.farmer.farmer import Farmer, increment_pool_stats, strip_old_entries
from xxch.full_node.full_node_api import FullNodeAPI
from xxch.harvester.harvester_api import HarvesterAPI
from xxch.protocols import farmer_protocol, harvester_protocol
from xxch.protocols.farmer_protocol import DeclareProofOfSpace, SignedValues
from xxch.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
from xxch.protocols.pool_protocol import (
    PoolErrorCode,
    PostPartialPayload,
    PostPartialRequest,
...
from xxch.server.server import ssl_context_for_root
from xxch.server.ws_connection import WSXxchConnection
from xxch.ssl.create_ssl import get_mozilla_ca_crt
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: WSXxchConnection
    ) -> None:
        """
    ...
    @api_request(peer_required=True)
    async def farming_info(self, request: farmer_protocol.FarmingInfo, peer: WSXxchConnection) -> None:
        self.farmer.state_changed(
            "new_farming_info",
            {
                "farming_info": {
    ...
    @api_request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSXxchConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request(peer_required=True)
    async def plot_sync_start(self, message: PlotSyncStart, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request(peer_required=True)
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request(peer_required=True)
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request(peer_required=True)
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request(peer_required=True)
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request(peer_required=True)
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: WSXxchConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request(peer_required=True)
    async def plot_sync_done(self, message: PlotSyncDone, peer: WSXxchConnection) -> None:
