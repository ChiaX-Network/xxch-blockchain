...
from xxch.consensus.block_creation import unfinished_block_to_full_block
from xxch.consensus.block_record import BlockRecord
from xxch.consensus.blockchain import AddBlockResult, Blockchain, BlockchainMutexPriority, StateChangeSummary
from xxch.consensus.blockchain_interface import BlockchainInterface
from xxch.consensus.constants import ConsensusConstants
from xxch.consensus.cost_calculator import NPCResult
from xxch.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from xxch.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from xxch.consensus.multiprocess_validation import PreValidationResult
from xxch.consensus.pot_iterations import calculate_sp_iters
from xxch.full_node.block_store import BlockStore
from xxch.full_node.bundle_tools import detect_potential_template_generator
from xxch.full_node.coin_store import CoinStore
from xxch.full_node.full_node_api import FullNodeAPI
from xxch.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from xxch.full_node.hint_management import get_hints_and_subscription_coin_ids
from xxch.full_node.hint_store import HintStore
from xxch.full_node.mempool_manager import MempoolManager
from xxch.full_node.signage_point import SignagePoint
from xxch.full_node.stake_record_store import StakeRecordStore
from xxch.full_node.subscriptions import PeerSubscriptions
from xxch.full_node.sync_store import Peak, SyncStore
from xxch.full_node.tx_processing_queue import TransactionQueue
from xxch.full_node.weight_proof import WeightProofHandler
from xxch.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from xxch.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from xxch.protocols.protocol_message_types import ProtocolMessageTypes
from xxch.protocols.wallet_protocol import CoinState, CoinStateUpdate
from xxch.rpc.rpc_server import StateChangedProtocol
from xxch.server.node_discovery import FullNodePeers
from xxch.server.outbound_message import Message, NodeType, make_msg
from xxch.server.peer_store_resolver import PeerStoreResolver
from xxch.server.server import XxchServer
from xxch.server.ws_connection import WSXxchConnection
from xxch.types.blockchain_format.classgroup import ClassgroupElement
...
from xxch.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from xxch.types.generator_types import BlockGenerator
...
from xxch.types.transaction_queue_entry import TransactionQueueEntry
from xxch.types.unfinished_block import UnfinishedBlock
from xxch.types.weight_proof import WeightProof
from xxch.util import cached_bls
from xxch.util.bech32m import encode_puzzle_hash
from xxch.util.check_fork_next_block import check_fork_next_block
from xxch.util.condition_tools import pkm_pairs
from xxch.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from xxch.util.db_synchronous import db_synchronous_on
from xxch.util.db_version import lookup_db_version, set_db_version_async
from xxch.util.db_wrapper import DBWrapper2, manage_connection
from xxch.util.errors import ConsensusError, Err, TimestampError, ValidationError
from xxch.util.ints import uint8, uint32, uint64, uint128
from xxch.util.limited_semaphore import LimitedSemaphore
from xxch.util.log_exceptions import log_exceptions
from xxch.util.path import path_from_root
from xxch.util.profiler import mem_profile_task, profile_task
from xxch.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    ...
    _stake_record_store: Optional[StakeRecordStore]
                ...
                async with conn.execute(
                    ...
                    if len(list(await cur.fetchall())) == 0:
                        try:
                            ...
                            # it could be a database created with "xxch init", which is
    ...
    def set_server(self, server: XxchServer) -> None:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.xxch.cc")
        try:
    ...
    async def short_sync_batch(self, peer: WSXxchConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSXxchConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSXxchConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSXxchConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSXxchConnection) -> None:
        """
    ...
    def on_disconnect(self, connection: WSXxchConnection) -> None:
    ...
    async def request_validate_wp(
        ...
        weight_proof_peer: WSXxchConnection = random.choice(peers_with_peak)
        self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSXxchConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSXxchConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSXxchConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSXxchConnection, List[FullBlock]]]]
        ) -> None:
            advanced_peak: bool = False
            while True:
                res: Optional[Tuple[WSXxchConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSXxchConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSXxchConnection]:
    ...
    async def add_block_batch(
        ...
        if agg_state_change_summary is not None:
            ...
            self.log.debug(
                f"Total time for {len(blocks_to_validate)} blocks: {time.monotonic() - pre_validate_start:0.2f}, "
                f"advanced: True"
            )
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSXxchConnection] = None,
    ...
    async def add_unfinished_block(
        ...
        if await self.blockchain.check_stake_coefficient(block.reward_chain_block.proof_of_space):
            self.log.error("unfinished_block staking coefficient error")
            return None
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'xxch')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSXxchConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSXxchConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSXxchConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSXxchConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSXxchConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSXxchConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Optional[Any] = await peer.call_api(
