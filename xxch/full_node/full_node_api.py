...
from xxch.full_node.fee_estimate import FeeEstimate, FeeEstimateGroup, fee_rate_v2_to_v1
from xxch.full_node.fee_estimator_interface import FeeEstimatorInterface
from xxch.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_puzzle_and_solution_for_coin
from xxch.full_node.signage_point import SignagePoint
from xxch.full_node.tx_processing_queue import TransactionQueueFull
from xxch.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from xxch.protocols.full_node_protocol import RejectBlock, RejectBlocks
from xxch.protocols.protocol_message_types import ProtocolMessageTypes
from xxch.protocols.wallet_protocol import (
    CoinState,
...
from xxch.server.outbound_message import Message, make_msg
from xxch.server.server import XxchServer
from xxch.server.ws_connection import WSXxchConnection
from xxch.types.block_protocol import BlockInfo
...
from xxch.types.blockchain_format.proof_of_space import verify_and_get_quality_string
...
from xxch.util.full_block_utils import header_block_from_block
from xxch.util.generator_tools import get_block_header, tx_removals_and_additions
from xxch.util.hash import std_hash
from xxch.util.ints import uint8, uint32, uint64, uint128
from xxch.util.limited_semaphore import LimitedSemaphoreFullError
from xxch.util.merkle_set import MerkleSet
from xxch.util.prev_transaction_block import get_prev_transaction_block

if TYPE_CHECKING:
    from xxch.full_node.full_node import FullNode
else:
    FullNode = object
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: WSXxchConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: WSXxchConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSXxchConnection
    ) -> Optional[Message]:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSXxchConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSXxchConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSXxchConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSXxchConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSXxchConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSXxchConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSXxchConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSXxchConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSXxchConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSXxchConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSXxchConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSXxchConnection
    ) -> Message:
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSXxchConnection
    ) -> Message:
    ...
    def is_trusted(self, peer: WSXxchConnection) -> bool:
    ...
    async def request_stake_coefficients(
        ...
        return make_msg(
            ProtocolMessageTypes.respond_stake_coefficients,
            farmer_protocol.FarmerStakeCoefficients(stake_coefficients),
        )
    ...
    async def request_stake_farm_count(
        ...
        return make_msg(
            ProtocolMessageTypes.respond_stake_farm_count,
            wallet_protocol.RespondStakeFarmCount(count),
        )
