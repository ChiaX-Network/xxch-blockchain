...
from xxch.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from xxch.plotters.chiapos import get_chiapos_install_info, plot_xxch
from xxch.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in XXCH_ROOT.
    xxch_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within XXCH_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("xxch plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, xxch_plotter_options, "chiapos", "Create a plot with the default xxch plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'xxch plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'xxch plotters bladebit2' was integrated to 'xxch plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_xxch(args, xxch_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, xxch_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, xxch_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(xxch_root_path)
