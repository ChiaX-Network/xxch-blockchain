...
from xxch.simulator.simulator_protocol import FarmNewBlockProtocol
...
from xxch.util.misc import UInt32Range
from xxch.util.path import path_from_root
from xxch.util.streamable import Streamable, streamable
from xxch.util.ws_message import WsRpcMessage, create_payload_dict
...
from xxch.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
...
from xxch.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from xxch.wallet.nft_wallet.nft_wallet import NFTWallet
from xxch.wallet.nft_wallet.uncurry_nft import UncurriedNFT
...
from xxch.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xxch_target_address")
        pool_target = config["pool"].get("xxch_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from xxch.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `xxch keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from xxch.util.bech32m import bech32_decode, convertbits
        from xxch.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xxch")
        ...
        key = None
        if asset_id is not None and asset_id != "xxch":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XXCH.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                xxch_coins=xxch_coins,
                xxch_change_ph=xxch_change_ph,
