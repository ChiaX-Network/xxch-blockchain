...
from xxch.protocols.protocol_state_machine import message_requires_reply
from xxch.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from xxch.protocols.shared_protocol import protocol_version
from xxch.server.api_protocol import ApiProtocol
from xxch.server.introducer_peers import IntroducerPeers
from xxch.server.outbound_message import Message, NodeType
from xxch.server.ssl_context import private_ssl_paths, public_ssl_paths
from xxch.server.ws_connection import ConnectionCallback, WSXxchConnection
from xxch.types.blockchain_format.sized_bytes import bytes32
from xxch.types.peer_info import PeerInfo
from xxch.util.errors import Err, ProtocolError
from xxch.util.ints import uint16
from xxch.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from xxch.util.ssl_check import verify_ssl_certs_and_keys
...
class XxchServer:
    ...
    all_connections: Dict[bytes32, WSXxchConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        xxch_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> XxchServer:
        ...
        xxch_ca_crt_path, xxch_ca_key_path = xxch_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=xxch_ca_crt_path,
                ca_key=xxch_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSXxchConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("XxchServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSXxchConnection] = None
        try:
            connection = WSXxchConnection.create(
    ...
    async def connection_added(
        self, connection: WSXxchConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSXxchConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSXxchConnection, trusted_peers: Dict[str, Any]) -> bool:
