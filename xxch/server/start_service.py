...
from xxch.daemon.server import service_launch_lock_path
from xxch.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from xxch.server.api_protocol import ApiProtocol
from xxch.server.xxch_policy import set_xxch_policy
from xxch.server.outbound_message import NodeType
from xxch.server.server import XxchServer
from xxch.server.ssl_context import xxch_ssl_ca_paths, private_ssl_ca_paths
from xxch.server.upnp import UPnP
from xxch.server.ws_connection import WSXxchConnection
from xxch.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from xxch.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSXxchConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"xxch-blockchain version: {xxch_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        xxch_ca_crt, xxch_ca_key = xxch_ssl_ca_paths(root_path, self.config)
        ...
        self._server = XxchServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"xxch_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for XxchServer to be closed")
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_xxch_policy(connection_limit)
