...
from xxch.consensus.condition_costs import ConditionCost
from xxch.consensus.constants import ConsensusConstants
from xxch.consensus.default_constants import DEFAULT_CONSTANTS
from xxch.consensus.deficit import calculate_deficit
from xxch.consensus.full_block_to_block_record import block_to_block_record
from xxch.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from xxch.consensus.pot_iterations import (
    calculate_ip_iters,
...
from xxch.consensus.vdf_info_computation import get_signage_point_vdf_info
...
from xxch.plotting.create_plots import PlotKeys, create_plots
from xxch.plotting.manager import PlotManager
from xxch.plotting.util import (
    Params,
    PlotRefreshEvents,
    PlotRefreshResult,
...
from xxch.server.server import ssl_context_for_client
from xxch.simulator.socket import find_available_listen_port
from xxch.simulator.ssl_certs import (
...
from xxch.simulator.time_out_assert import time_out_assert_custom_interval
from xxch.simulator.wallet_tools import WalletTool
from xxch.ssl.create_ssl import create_all_ssl
from xxch.types.blockchain_format.classgroup import ClassgroupElement
from xxch.types.blockchain_format.coin import Coin, hash_coin_ids
from xxch.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from xxch.types.blockchain_format.pool_target import PoolTarget
from xxch.types.blockchain_format.program import INFINITE_COST, Program
from xxch.types.blockchain_format.proof_of_space import (
    ProofOfSpace,
...
from xxch.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
...
from xxch.types.blockchain_format.slots import (
...
from xxch.types.generator_types import BlockGenerator, CompressorArg
from xxch.types.spend_bundle import SpendBundle
from xxch.types.stake_record import create_stake_farm_rewards
...
from xxch.util.block_cache import BlockCache
from xxch.util.config import (
    config_path_for_filename,
    create_default_xxch_config,
    load_config,
    lock_config,
    override_config,
    save_config,
...
from xxch.util.keychain import Keychain, bytes_to_mnemonic
from xxch.util.prev_transaction_block import get_prev_transaction_block
from xxch.util.ssl_check import fix_ssl
from xxch.util.vdf_prover import get_vdf_info_and_proof
...
from xxch.wallet.puzzles.load_clvm import load_serialized_clvm_maybe_recompile

GENERATOR_MOD: SerializedProgram = load_serialized_clvm_maybe_recompile(
    "rom_bootstrap_generator.clsp", package_or_requirement="xxch.consensus.puzzles"
)

DESERIALIZE_MOD = load_serialized_clvm_maybe_recompile(
    "chialisp_deserialisation.clsp", package_or_requirement="xxch.consensus.puzzles"
)
...
class BlockTools:
    ...
    def __init__(
        ...
        if automated_testing:
            ...
            create_default_xxch_config(root_path)
            create_all_ssl(
                root_path,
        ...
        try:
            if self.local_keychain:
                ...
                keychain_proxy = await connect_to_keychain_and_validate(
                    self.root_path, self.log, user="testing-1.8.0", service="xxch-testing-1.8.0"
                )
            ...
            if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
                raise RuntimeError("Keys not generated. Run `xxch keys generate`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "xxch")
            ...
            if num_empty_slots_added < skip_slots:
                ...
                for signage_point_index in range(0, constants.NUM_SPS_SUB_SLOT - constants.NUM_SP_INTERVALS_EXTRA):
                    ...
                    for required_iters, proof_of_space in sorted(qualified_proofs, key=lambda t: t[0]):
                        ...
                        additions = None
                        removals = None
                        stake_farm_records_dict = None
                        stake_lock_records_dict = None
    ...
    def get_pospaces_for_challenge(
        ...
        for plot_info in self.plot_manager.plots.values():
            ...
            if passes_plot_filter(prefix_bits, plot_id, challenge_hash, signage_point):
                ...
                stake_coefficient = uint64(STAKE_PER_COEFFICIENT)
...
def get_full_block_and_block_record(
    ...
    block_generator: Optional[BlockGenerator],
    aggregate_signature: G2Element,
    stake_farm_records_dict: Optional[Dict[bytes32, Dict[bytes32, int]]],
    stake_lock_records_dict: Optional[Dict[bytes32, Dict[bytes32, int]]],
...
def create_test_foliage(
    ...
    stake_lock_records_dict: Dict[bytes32, Dict[bytes32, int]],
    additions: List[Coin],
    removals: List[Coin],
    ...
    Args:
        ...
        stake_lock_records_dict: Stake fixed records dict
        ...
        if height > 0:
            ...
            if pool_reward > 0:
                pool_coin = create_pool_coin(
                    curr.height, curr.pool_puzzle_hash, pool_reward, constants.GENESIS_CHALLENGE
                )
                reward_claims_incorporated += [pool_coin]
            ...
            reward_claims_incorporated += [farmer_coin, community_coin]

            stake_records = stake_farm_records_dict.get(curr.header_hash)
            if stake_records is not None and len(stake_records) > 0:
                reward_claims_incorporated += create_stake_farm_rewards(constants, stake_records, curr.height)
...
def create_test_unfinished_block(
    ...
    stake_lock_records_dict: Optional[Dict[bytes32, Dict[bytes32, int]]] = None,
