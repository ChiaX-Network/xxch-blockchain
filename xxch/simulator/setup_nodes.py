...
from xxch.daemon.server import WebSocketServer
...
from xxch.util.ints import uint16, uint32
...
SimulatorsAndWallets = Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, XxchServer]], BlockTools]
...
) -> AsyncIterator[Tuple[FullNodeAPI, FullNodeAPI, XxchServer, XxchServer, BlockTools]]:
    """
...
) -> AsyncIterator[Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, XxchServer]], BlockTools]]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system(
    ...
    Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord, TimelordAPI], object, object, Any, XxchServer]
]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
