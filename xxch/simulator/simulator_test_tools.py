...
from xxch.simulator.start_simulator import async_main as start_simulator_main
...
from xxch.util.config import create_default_xxch_config, load_config, save_config
...
from xxch.wallet.derive_keys import master_sk_to_wallet_sk
...
def create_config(
    ...
    # create xxch directories
    create_default_xxch_config(xxch_root)
    create_all_ssl(
        xxch_root,
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txxch")
...
async def get_full_xxch_simulator(
    ...
    """
    A xxch root Path is required.
    The xxch root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the xxch test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(xxch_root)):
        ...
        crt_path = xxch_root / config["daemon_ssl"]["private_crt"]
        key_path = xxch_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = xxch_root / config["private_ssl_ca"]["crt"]
        ca_key_path = xxch_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(xxch_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            await ws_server.setup_process_global_state(signal_handlers=signal_handlers)
            async with ws_server.run():
                async for simulator in start_simulator(xxch_root, automated_testing):
                    yield simulator, xxch_root, config, mnemonic, fingerprint, keychain
