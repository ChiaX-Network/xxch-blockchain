...
# the output will be printed to your xxch root path, e.g. ~/.xxch/mainnet/profile/
# to analyze the profile, run:

#   python xxch/utils/profiler.py ~/.xxch/mainnet/profile | less -r

# this will print CPU usage of the xxch full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python xxch/utils/profiler.py ~/.xxch/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int) -> None:
        ...
        output_file = "xxch-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
