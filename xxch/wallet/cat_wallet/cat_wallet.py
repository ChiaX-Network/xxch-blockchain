...
from xxch.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
from xxch.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from xxch.wallet.cat_wallet.lineage_store import CATLineageStore
from xxch.wallet.coin_selection import select_coins
from xxch.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from xxch.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from xxch.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            xxch_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSXxchConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_xxch_tx(
        ...
        if fee > amount_to_claim:
            xxch_coins = await self.standard_wallet.select_coins(
                fee,
            ...
            message = None
            for spend in xxch_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XXCH as part of this
        regular_xxch_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_xxch_to_claim = payment_amount

        need_xxch_transaction = (fee > 0 or regular_xxch_to_claim > 0) and (fee - regular_xxch_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_xxch_transaction:
                    if fee > regular_xxch_to_claim:
                        xxch_tx, _ = await self.create_tandem_xxch_tx(
                            fee,
                            uint64(regular_xxch_to_claim),
                            tx_config,
                    ...
                    elif regular_xxch_to_claim > fee:
                        xxch_tx, _ = await self.create_tandem_xxch_tx(
                            fee,
                            uint64(regular_xxch_to_claim),
                            tx_config,
        ...
        xxch_spend_bundle = SpendBundle([], G2Element())
        if xxch_tx is not None and xxch_tx.spend_bundle is not None:
            xxch_spend_bundle = xxch_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if xxch_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=xxch_tx.confirmed_at_height,
                    created_at_time=xxch_tx.created_at_time,
                    to_puzzle_hash=xxch_tx.to_puzzle_hash,
                    amount=xxch_tx.amount,
                    fee_amount=xxch_tx.fee_amount,
                    confirmed=xxch_tx.confirmed,
                    sent=xxch_tx.sent,
                    spend_bundle=None,
                    additions=xxch_tx.additions,
                    removals=xxch_tx.removals,
                    wallet_id=xxch_tx.wallet_id,
                    sent_to=xxch_tx.sent_to,
                    trade_id=xxch_tx.trade_id,
                    type=xxch_tx.type,
                    name=xxch_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
