...
from xxch.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle
from xxch.wallet.dao_wallet.dao_info import DAORules, ProposalType
from xxch.wallet.puzzles.load_clvm import load_clvm
from xxch.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import MOD
from xxch.wallet.singleton import get_singleton_struct_for_id
from xxch.wallet.uncurried_puzzle import UncurriedPuzzle
...
DAO_CAT_TAIL: Program = load_clvm(
    "genesis_by_coin_id_or_singleton.clsp", package_or_requirement="xxch.wallet.cat_wallet.puzzles"
)
...
def get_spend_p2_singleton_puzzle(
    treasury_id: bytes32, xxch_conditions: Optional[List], asset_conditions: Optional[List[Tuple]]  # type: ignore
) -> Program:
    # TODO: typecheck get_spend_p2_singleton_puzzle arguments
    # TODO: add tests for get_spend_p2_singleton_puzzle: pass xxch_conditions as Puzzle, List and ConditionWithArgs
    #

    # CAT_MOD_HASH
    # CONDITIONS  ; XXCH conditions, to be generated by the treasury
...
def generate_simple_proposal_innerpuz(
    ...
    for recipient_puzhash, amount, asset_type in zip(recipient_puzhashes, amounts, asset_types):
        if asset_type:
            ...
            xxch_conds.append([51, recipient_puzhash, amount])
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(xxch_conds), Program.to(cat_conds))
    return puzzle
...
async def generate_mint_proposal_innerpuz(
    ...
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(xxch_conditions), [])
    return puzzle
