...
from xxch.wallet.did_wallet import did_wallet_puzzles
from xxch.wallet.did_wallet.did_info import DIDInfo
from xxch.wallet.lineage_proof import LineageProof
from xxch.wallet.nft_wallet import nft_puzzles
from xxch.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from xxch.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from xxch.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from xxch.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
...
from xxch.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
from xxch.wallet.util.wallet_types import WalletType
...
from xxch.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSXxchConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSXxchConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if xxch_tx is not None and xxch_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, xxch_tx.spend_bundle])
            xxch_tx = dataclasses.replace(xxch_tx, spend_bundle=None)
        ...
        if xxch_tx is not None:
            tx_list.append(xxch_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            xxch_tx = await self.standard_wallet.create_tandem_xxch_tx(
        ...
        return nft_spend_bundle, xxch_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XXCH'} to offer")
    ...
    async def mint_from_did(
        ...
        xxch_coins: Optional[Set[Coin]] = None,
        xxch_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xxch spend that funds the transactions and pays fees.
        ...
        :param xxch_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xxch_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xxch spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xxch coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xxch_coins is None:
            xxch_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(xxch_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                xxch.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), xxch.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, xxch.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the xxch spend to fund the minting.
        spend_value = sum([coin.amount for coin in xxch_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xxch_spends = []
        if xxch_change_ph is None:
            xxch_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xxch_payment = Payment(xxch_change_ph, change, [xxch_change_ph])

        first = True
        for xxch_coin in xxch_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xxch_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xxch_coins]
                message_list.append(Coin(xxch_coin.name(), xxch_payment.puzzle_hash, xxch_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xxch_coins) > 1:
                    xxch_announcement: Optional[Set[bytes]] = {message}
                else:
                    xxch_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[xxch_payment],
                    fee=fee,
                    coin_announcements=xxch_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xxch_coin.name(), message).name()
            ...
            xxch_spends.append(CoinSpend(xxch_coin, puzzle, solution))
        xxch_spend = await self.wallet_state_manager.sign_transaction(xxch_spends)
        ...
        did_full_puzzle: Program = xxch.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xxch_spend, *eve_spends])
        return total_spend

    async def mint_from_xxch(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        """
        Minting NFTs from a single XXCH spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xxch_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XXCH to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XXCH spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XXCH spend
            ...
            # Annnouncements for eve spend. These are asserted by the xxch spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xxch_coin in xxch_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[xxch_payment] + primaries,
                    fee=fee,
                    coin_announcements=xxch_announcement if len(xxch_coins) > 1 else None,
