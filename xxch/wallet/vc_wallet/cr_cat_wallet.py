...
from xxch.types.blockchain_format.coin import Coin, coin_as_list
...
from xxch.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from xxch.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from xxch.wallet.util.tx_config import TXConfig
...
from xxch.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSXxchConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_xxch_transaction:
                    if fee > regular_xxch_to_claim:
                        ...
                        xxch_tx, xxch_announcement = await self.create_tandem_xxch_tx(
                            fee,
                            uint64(regular_xxch_to_claim),
                            tx_config,
                        )
                        assert xxch_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={xxch_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [xxch_tx.spend_bundle]
            ...
            [*vc_txs, *([xxch_tx] if xxch_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant XXCH coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(xxch_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if xxch_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, xxch_tx.spend_bundle])
        else:
            xxch_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(xxch_tx, spend_bundle=None),) if xxch_tx is not None else []),
        ]
