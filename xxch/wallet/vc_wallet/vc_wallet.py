...
from xxch.wallet.puzzle_drivers import Solver
from xxch.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import solution_for_conditions
...
from xxch.wallet.vc_wallet.cr_cat_drivers import CRCAT, CRCATSpend, ProofsChecker, construct_pending_approval_state
from xxch.wallet.vc_wallet.vc_drivers import VerifiedCredential
from xxch.wallet.vc_wallet.vc_store import VCProofs, VCRecord, VCStore
...
if TYPE_CHECKING:
    from xxch.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSXxchConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            xxch_tx = await self.wallet_state_manager.main_wallet.create_tandem_xxch_tx(
        ...
        if xxch_tx is not None and xxch_tx.spend_bundle is not None:
            spend_bundles.append(xxch_tx.spend_bundle)
            tx_list.append(dataclasses.replace(xxch_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            xxch_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_xxch_tx(
                fee, tx_config, vc_announcement
            )
            assert tx.spend_bundle is not None
            assert xxch_tx.spend_bundle is not None
            tx = dataclasses.replace(tx, spend_bundle=SpendBundle.aggregate([xxch_tx.spend_bundle, tx.spend_bundle]))
