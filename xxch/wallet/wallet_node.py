...
from xxch.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from xxch.wallet.puzzles.clawback.metadata import AutoClaimSettings
...
from xxch.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from xxch.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from xxch.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSXxchConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'xxch keys' program.")
            return None
    ...
    def on_disconnect(self, peer: WSXxchConnection) -> None:
    ...
    async def state_update_received(self, request: CoinStateUpdate, peer: WSXxchConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSXxchConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSXxchConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSXxchConnection]:
        """
        ...
        synced_and_trusted: List[WSXxchConnection] = []
        synced: List[WSXxchConnection] = []
        trusted: List[WSXxchConnection] = []
        neither: List[WSXxchConnection] = []
        all_nodes: List[WSXxchConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSXxchConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSXxchConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSXxchConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSXxchConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSXxchConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSXxchConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSXxchConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSXxchConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSXxchConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSXxchConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSXxchConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
