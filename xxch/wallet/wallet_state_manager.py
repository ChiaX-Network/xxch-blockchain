...
from xxch.consensus.block_rewards import (
...
from xxch.data_layer.dl_wallet_store import DataLayerStore
from xxch.pools.pool_puzzles import (
...
from xxch.types.stake_value import STAKE_FARM_MIN, get_stake_value
from xxch.util.bech32m import encode_puzzle_hash
from xxch.util.db_synchronous import db_synchronous_on
from xxch.util.db_wrapper import DBWrapper2
from xxch.util.errors import Err
...
from xxch.util.misc import UInt32Range, UInt64Range, VersionedBlob
from xxch.util.path import path_from_root
from xxch.util.streamable import Streamable
from xxch.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from xxch.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from xxch.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from xxch.wallet.cat_wallet.cat_wallet import CATWallet
from xxch.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from xxch.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
...
from xxch.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
from xxch.wallet.derivation_record import DerivationRecord
...
from xxch.wallet.did_wallet.did_info import DIDCoinData
from xxch.wallet.did_wallet.did_wallet import DIDWallet
from xxch.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from xxch.wallet.key_val_store import KeyValStore
from xxch.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from xxch.wallet.nft_wallet.nft_wallet import NFTWallet
from xxch.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from xxch.wallet.notification_manager import NotificationManager
from xxch.wallet.outer_puzzles import AssetType
from xxch.wallet.payment import Payment
from xxch.wallet.puzzle_drivers import PuzzleInfo
from xxch.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from xxch.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from xxch.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from xxch.wallet.puzzles.stake.drivers import match_stake_puzzle, generate_stake_spend_bundle
from xxch.wallet.puzzles.stake.metadata import StakeMetadata, StakeVersion
from xxch.wallet.sign_coin_spends import sign_coin_spends
from xxch.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from xxch.wallet.trade_manager import TradeManager
from xxch.wallet.trading.trade_status import TradeStatus
from xxch.wallet.transaction_record import TransactionRecord, TransactionRecordOld
from xxch.wallet.uncurried_puzzle import uncurry_puzzle
from xxch.wallet.util.address_type import AddressType
from xxch.wallet.util.compute_hints import compute_spend_hints_and_additions
...
from xxch.wallet.util.query_filter import HashFilter
from xxch.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from xxch.wallet.util.tx_config import TXConfig, TXConfigLoader
from xxch.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from xxch.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from xxch.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from xxch.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from xxch.wallet.vc_wallet.vc_drivers import VerifiedCredential
from xxch.wallet.vc_wallet.vc_store import VCStore
from xxch.wallet.vc_wallet.vc_wallet import VCWallet
from xxch.wallet.wallet import Wallet
from xxch.wallet.wallet_blockchain import WalletBlockchain
from xxch.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from xxch.wallet.wallet_coin_store import WalletCoinStore
from xxch.wallet.wallet_info import WalletInfo
from xxch.wallet.wallet_interested_store import WalletInterestedStore
from xxch.wallet.wallet_nft_store import WalletNftStore
from xxch.wallet.wallet_pool_store import WalletPoolStore
from xxch.wallet.wallet_protocol import WalletProtocol
from xxch.wallet.wallet_puzzle_store import WalletPuzzleStore
from xxch.wallet.wallet_retry_store import WalletRetryStore
from xxch.wallet.wallet_transaction_store import WalletTransactionStore
from xxch.wallet.wallet_user_store import WalletUserStore

TWalletType = TypeVar("TWalletType", bound=WalletProtocol[Any])

if TYPE_CHECKING:
    from xxch.wallet.wallet_node import WalletNode
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSXxchConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ) -> Tuple[Optional[WalletIdentifier], Optional[Streamable]]:
        created_height = uint32(coin_state.created_height)
        if (
            self.is_pool_reward(created_height, coin_state.coin)
            or self.is_farmer_reward(created_height, coin_state.coin)
            or self.is_stake_farm_reward(created_height, coin_state.coin)
            or self.is_stake_lock_reward(created_height, coin_state.coin)
        ):
            return None, None
        ...
        assert parent_coin_state.spent_height == created_height
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the XXCH coin
        ...
        if fee > 0:
            xxch_tx = await self.main_wallet.create_tandem_xxch_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert xxch_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, xxch_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xxch_spam_amount = self.config.get("xxch_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xxch_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xxch_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xxch_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSXxchConnection) -> Optional[bytes32]:
        # Get minter DID
        ...
        for coin in unspent_coins.records:
            try:
                ...
                if current_timestamp - coin_timestamp >= metadata.time_lock:
                    ...
                    if len(stake_coins) >= batch_size:
                        await self.spend_stake_coins(stake_coins, tx_fee, tx_config)
                        stake_coins = {}
            except Exception as e:
